...
namespace metrics.Reporting
{
    ...
    public class JsonReportFormatter : IReportFormatter
    {
        ...
        {
            return Serializer.Serialize(Metrics.All);
        }
    }
    ...
    public class HumanReadableReportFormatter : IReportFormatter
    {
        ...
        protected void WriteHistogram(StringBuilder sb, HistogramMetric histogram)
        {
            ...
            sb.AppendFormat("               min = %{0:2}\n", histogram.Min);
            sb.AppendFormat("               max = %{0:2}\n", histogram.Max);
            sb.AppendFormat("              mean = %{0:2}\n", histogram.Mean);
            sb.AppendFormat("            stddev = %{0:2}\n", histogram.StdDev);
            sb.AppendFormat("            median = %{0:2}\n", percentiles[0]);
            sb.AppendFormat("              75%% <= %{0:2}\n", percentiles[1]);
            sb.AppendFormat("              95%% <= %{0:2}\n", percentiles[2]);
            sb.AppendFormat("              98%% <= %{0:2}\n", percentiles[3]);
            sb.AppendFormat("              99%% <= %{0:2}\n", percentiles[4]);
            sb.AppendFormat("            99.9%% <= %{0:2}\n", percentiles[5]);
        }
