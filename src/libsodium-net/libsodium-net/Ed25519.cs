...
namespace Sodium
{
  public static class Ed25519
  {
    ...
    public static void KeyPairFromPrivateKey(out byte[] publicKey, byte[] privateKey)
    {
      // We cannot derive a public key directly, so we need to regenerate the wallet keypair,
      // Then use that to generate an address for the given privateKey, which is normally used as a 
