    ...
    public class JsonParser
    {
        ...
        public static IDictionary<string, object> FromJson(string json)
        {
            ...
            switch (type)
            {
                case JsonToken.LeftBrace:
					// MonoTouch barfs on result.Single().Value...
					// http://www.iappnow.com/2011/06/29/monotouch-crash-dictionary-firstordefault-type-initializer-predicateof/
                    
					var enumerator = result.GetEnumerator();
					enumerator.MoveNext();
				
					var @object = (IDictionary<string, object>)enumerator.Current.Value;
                    return @object;
            }
        ...
        internal static IDictionary<string, object> GetBagForObject(Type type, object instance)
        {
            ...
            foreach (var info in map)
            {
                ...
                var name = NormalizePropertyName(info.Name, '_');
        ...
        internal static string NormalizePropertyName(string str, char spacer)
        {
            ...
            value.Append(char.ToUpper(str[0]));
            for (var i = 1; i < str.Length; i++)
            {
                ...
                {
                    value.Append(spacer);
                    value.Append(char.ToLower(str[i]));
                }
            }
